...
class Net(hk.Module):
    ...
    def _msg_passing_step(
        ...
        if self.decode_hints and not first_step:
            decoded_hint = decoders.postprocess(spec, mp_state.hint_preds)
        ...
        if self.decode_diffs:
            if self.decode_hints:
                if hints[0].data.shape[0] == 1 or repred:
                    diff_preds = {}
                    for loc in [_Location.NODE, _Location.EDGE, _Location.GRAPH]:
                        diff_preds[loc] = (diff_logits[loc] > 0.0).astype(
                            jnp.float32
                        ) * 1.0
                else:
                    diff_preds = gt_diffs
                for hint in hints:
                    prev_hint = (
                        hint.data[0] if first_step else mp_state.hint_preds[hint.name]
                    )
                    ...
                    hint_preds[hint.name] = (
                        cur_diffs * hint_preds[hint.name]
                        + (1.0 - cur_diffs) * prev_hint
                    )
    ...
    def __call__(
        self, features_list: List[_Features], repred: bool, algorithm_index: int
    ):
        ...
        (self.encoders, self.decoders, self.diff_decoders) = (
        ...
        if self.use_lstm:
            self.lstm = hk.LSTM(hidden_size=self.hidden_dim, name="processor_lstm")
        ...
        for algorithm_index, features in zip(algorithm_indices, features_list):
            ...
            mp_state, _ = self._msg_passing_step(
                mp_state, i=0, first_step=True, **common_args
            )
    ...
    def _one_step_pred(
        ...
        if nxt_e_hidden is not None:
            nxt_e_hidden = hk.dropout(
                hk.next_rng_key(), self._dropout_prob, nxt_e_hidden
            )
        if nxt_g_hidden is not None:
            nxt_g_hidden = hk.dropout(
                hk.next_rng_key(), self._dropout_prob, nxt_g_hidden
            )
...
class NetChunked(Net):
    ...
    def _msg_passing_step(
        ...
        if init_mp_state:
            ...
            if self.decode_hints:
                ...
                elif self._hint_teacher_forcing_noise == 0.0:
                ...
                hints_for_pred = []
                for h in hints:
                    hints_for_pred.append(
                        probing.DataPoint(
                            name=h.name,
                            location=h.location,
                            type_=h.type_,
                            data=jnp.where(
                                _expand_to(is_first | force_mask, h.data),
                                h.data,
                                decoded_hints[h.name].data,
                            ),
                        )
                    )
            else:
        ...
        if self.use_lstm:
            ...
            lstm_state = None
        (hiddens, output_preds, hint_preds, diff_logits, lstm_state) = (
            self._one_step_pred(
                inputs,
                hints_for_pred,
                hiddens,
        ...
        if self.decode_diffs and self.decode_hints:
            ...
            for hint in hints:
                ...
                prev_hint = jnp.where(
                    _expand_to(is_first, hint_data),
                    hint_data,
                    prev_hint_preds[hint.name],
                )
                cur_diffs = _expand_to(diff_preds[hint.location], hint_data)
                hint_preds[hint.name] = jnp.where(
                    cur_diffs, hint_preds[hint.name], prev_hint
                )
    ...
    def __call__(
        self,
        features_list: List[_FeaturesChunked],
        ...
        # is used only to init parameters.
        return (
            scan_output.output_preds,
            scan_output.hint_preds,
            scan_output.diff_logits,
            scan_output.gt_diffs,
        ), mp_state
