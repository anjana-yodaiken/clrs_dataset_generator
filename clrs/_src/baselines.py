...
class BaselineModel(model.Model):
    ...
    def predict(
        ...
        return decoders.postprocess(self._spec[algorithm_index], outs), (
            hint_preds,
            diff_logits,
            gt_diff,
        )
    ...
    def _loss(self, params, rng_key, feedback, algorithm_index):
        ...
        (output_preds, hint_preds, diff_logits, gt_diffs) = self.net_fn_apply(
            params,
            rng_key,
            [feedback.features],
            repred=False,
    ...
    def update(
        ...
        lss, grads = jax.value_and_grad(self.jitted_loss)(
...
class BaselineModelChunked(BaselineModel):
    ...
    def _init_mp_state(self, features_list: List[_FeaturesChunked], rng_key: _Array):
        def _empty_mp_state():
    ...
    def init(
        self, features: Union[_FeaturesChunked, List[_FeaturesChunked]], seed: _Seed
    ):
        if not isinstance(features, list):
            assert len(self._spec) == 1
            features = [features]
    ...
    def update(
        ...
        def loss(params, rng_key, feedback):
            ((output_preds, hint_preds, diff_logits, gt_diffs), mp_state) = (
                self.net_fn_apply(
                    params,
                    rng_key,
                    [feedback.features],
...
def filter_null_grads(grads, opt, opt_state, opt_state_skeleton):
    ...
    masked_grads = jax.tree_map(lambda x: x if jnp.abs(x).sum() > 0.0 else None, grads)
    ...
    updates = unflatten(flat_updates, jax.tree_map(lambda x: 0.0, grads))
